...
/*     public function query($sql,$parse=false) {
    ...
    public function query($sql,$parse=false) {
        ...
        $time_id = strtotime(date('Y-m-d H:i:s'));
        $start_time = date('Y-m-d H:i:s');
        $result = $this->db->query($sql);
        $end_time = date('Y-m-d H:i:s');
        
        $time_len = strtotime($end_time) - strtotime($start_time);
        $filedir = APP_PATH. '/Runtime' . DIRECTORY_SEPARATOR .
        'Logs' . DIRECTORY_SEPARATOR . 'sql';
        $filename = APP_PATH. '/Runtime' . DIRECTORY_SEPARATOR .
        'Logs' . DIRECTORY_SEPARATOR . 'sql' . DIRECTORY_SEPARATOR .
        date('Y_m_d') . '.log';
        if (!file_exists($filedir)){
            mkdir($filedir,0777);
        }
    
        $handle = fopen($filename, "a") or die("不能打开sql日志文件");
        if ($handle){
            fwrite($handle, $start_time."  ".$end_time."  ".$time_len."\r\n  ".$sql."\r\n\r\n\r\n\r\n");
            fclose($handle);
        }
